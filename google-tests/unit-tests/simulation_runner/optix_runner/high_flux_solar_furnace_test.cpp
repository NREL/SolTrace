...
TEST(HighFluxSolarFurnace, OptixRunnerCanRun)
{
    ...
    std::string sample_path =
        std::string(PROJECT_DIR) + std::string("/high_flux_solar_furnace_test.stinput");
    ...
    EXPECT_GT(total_hits, 0) << "No ray interactions recorded – GPU trace likely failed silently";
}
...
TEST(HighFluxSolarFurnace, OptixRunnerResultsConsistentWithNativeRunner)
{
    ...
    // Allow a 5% relative tolerance due to floating-point and stochastic differences.
    ...
    EXPECT_NEAR(ratio, 1.0, tolerance)
        ...
        << ") diverges from native hit count (" << native_total_hits << ") by more than "
        << (tolerance * 100) << "%";
}
