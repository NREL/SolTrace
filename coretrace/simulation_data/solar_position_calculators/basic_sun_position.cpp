                    ...
                    double *x, double *y, double *z)
{
    ...
    *x = -std::sin(Azimuth * D2R) * std::cos(Elevation * D2R);
    *y = std::sin(Elevation * D2R);
    *z = std::cos(Azimuth * D2R) * std::cos(Elevation * D2R);
...
void legacy_sun_position(double lat, double day, double hour, double* azimuth, double* zenith)
{
    ...
    */
    ...
    Declination = R2D * std::asin(0.39795 * std::cos(0.98563 * D2R * (day - 173)));
    HourAngle = 15 * (hour - 12);
    Elevation = R2D * std::asin(std::sin(Declination * D2R) * std::sin(lat * D2R) 
        + std::cos(Declination * D2R) * std::cos(HourAngle * D2R) * std::cos(lat * D2R));
    Azimuth = R2D * std::acos((std::sin(D2R * Declination) * std::cos(D2R * lat) 
        - std::cos(D2R * Declination) * std::sin(D2R * lat) * std::cos(D2R * HourAngle)) / std::cos(D2R * Elevation) + 0.0000000001);
    if (std::sin(HourAngle * D2R) > 0.0)
        Azimuth = 360 - Azimuth;
...
void duffie_sun_position(double lat, double lng, double tz, double day, double hour, double* azimuth, double* zenith)
{
    ...
    */
    ...
    double E = 229.2 * (0.000075 + 0.001868 * std::cos(D2R * B) - 0.032077 * std::sin(D2R * B) 
        - 0.014615 * std::cos(D2R * 2 * B) - 0.04089 * std::sin(D2R * 2 * B)); // [minutes] Equation 1.5.3 
    ...
    double Declination = R2D * (0.006918 - 0.399912 * std::cos(D2R * B) + 0.070257 * std::sin(D2R * B) 
        - 0.006758 * std::cos(D2R * 2 * B) + 0.000907 * std::sin(D2R * 2 * B) - 0.002697 * std::cos(D2R * 3 * B) 
        + 0.00148 * std::sin(D2R * 3 * B));
    double Zenith = R2D * std::acos(std::cos(D2R * lat) * std::cos(D2R * Declination) * std::cos(D2R * HourAngle) 
        + std::sin(D2R * lat) * std::sin(D2R * Declination));
    double ratio = (std::cos(D2R * Zenith) * std::sin(D2R * lat) 
        - std::sin(D2R * Declination)) / (std::sin(D2R * Zenith) * std::cos(D2R * lat));
    ...
    if (ratio > 1.0 && (ratio - 1.0) < 1.e-6)
        ...
        Azimuth = R2D * std::fabs(std::acos(ratio));
